...
{
   # Display Help
   echo "Generate a container image for the Clang toolchain with Buildah."
   echo
   echo "Syntax: clang-ssh-image.sh [-a|h]"
   echo "options:"
...
buildah run "$CONTAINER" /bin/sh -c 'microdnf install -y clang clang-tools-extra cmake libcxx-devel libcxxabi-devel lld lldb ninja-build python3 python3-pip python3-wheel python-unversioned-command tar --nodocs --setopt install_weak_deps=0'
