...
podman run --rm --arch $architecture --volume $mountpoint:/mnt:Z registry.fedoraproject.org/fedora:latest \
    bash -c "dnf -y install --installroot /mnt --releasever 34 bash black clang clang-tools-extra cmake coreutils git glibc-minimal-langpack libcxx-devel libcxxabi-devel lld lldb ninja-build python3 python3-pip python3-wheel python-unversioned-command tar --nodocs --setopt install_weak_deps=False"
or exit
